...
namespace BluefireDev\autoform;
...
class ReactCrudField
{
    ...
    public function type($type,$options=[]){
        ...
        if ($type === "map-showing"){
            $this->field[$this->name]['options']=$options[0]['options'];
            $this->field[$this->name]['show_field']=$options[0]['show_field'];
        }
    ...
    public function invisible($v){
        $this->field[$this->name]['invisible']=$v;
        return $this;
    }
    public function get(){
        return $this->field;
